...
/**
 * The Graph class allows a graph of coordinates to be entered and then
 * searched. Returning paths and nodes using Depth First Search, Breadth First
 * Search and A* Search.
 * 
 * @author Kyle Allen-Taylor
 * @param <A>
 *            Element the graph will consist of.
 */
...
public class Graph<A> {
	 ...
	 * 
	 * @param c
	 *            The coordinates of the node we wish to search for.
	 ...
	 *            The start node.
	 * @param p
	 *            The predicate the end node must satisfy.
	 * @param frontier
	 *            The frontier used in the search (so this will work on PC and
	 *            on the robot.
	 * @return The list of nodes to pass through to get to the target, or
	 *         nothing if it can't be reached.
	 */
	public IList<Node<A>> findPathFrom(Node<A> x, Node<A> p,
			IQueueContainer<A> frontier) {
		...
		while (!frontier.isEmpty()) {
			...
			if (!visited.contains(y)) {
				if (p.contents().equals(y.contents())) {
					IList<Node<A>> pathList = new Cons<Node<A>>(y,
							new Nil<Node<A>>());
